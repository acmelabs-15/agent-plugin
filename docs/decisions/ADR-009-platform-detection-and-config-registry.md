...
`@acmelabs-15/agent-plugin` targets 7 AI coding platforms (ADR-002). Each platform has different binary names, config directory paths, MCP config file locations, and content directory structures. Three interconnected problems must be solved:

1. **Platform detection**: How does the CLI determine which of the 7 platforms are installed on the user's machine? Binary-only checks via `which`/`where` miss GUI editors (Cursor, Windsurf) that users install without adding shell commands to PATH. Config-directory-only checks may false-positive from leftover directories of uninstalled software.
...
- Reliable detection across CLI-first tools (Claude Code, OpenCode, Amp) and GUI editors (Cursor, Windsurf, Kiro IDE) where binaries may not be in PATH
- Detection must complete within a bounded time (3 seconds total for all 7 platforms) to avoid blocking the install flow
...
- Adding support for a new platform (platform 8+) should require zero code changes for standard-format platforms
...
**Chosen option: Option A (dual detection)**, because neither signal alone is reliable across all 7 platforms.
...
All 7 platform checks run in parallel using `Promise.allSettled` with `Bun.spawn({ timeout: 3000 })` for binary checks. A platform is considered detected if either signal is positive. The detection result records which signal(s) matched (`binary`, `directory`, or `both`) for diagnostic purposes.

**Why dual**: Binary-only misses GUI editors (Cursor, Windsurf) where users install the desktop app but never run "Install shell command" from the Command Palette. Directory-only may false-positive from config directories left behind by uninstalled software. Both signals together provide high confidence. ANALYSIS-026 documents per-platform binary names and config paths across all 3 operating systems.

**Parallel execution**: `Promise.allSettled` ensures one platform timing out (e.g., network-mounted home directory slowing `which`) does not block detection of the other 6. The 3-second timeout per binary check is generous; `which`/`where` completes in under 50ms on typical systems.
...
| Amp | `amp` | `~/.config/amp/` | `~/.config/amp/` |
| Windsurf | `windsurf` | `~/.codeium/windsurf/` | `~/.codeium/windsurf/` |
...
The MCP server object itself (command, args, env) is standard across most platforms. Only the container file path and root key differ per platform (`mcpServers` for 5 platforms, `mcp` for 1, `amp.mcpServers` for 1). The namespaced key lives inside whichever root key the platform uses. Note: OpenCode uses a non-standard MCP format (command array instead of command+args, `environment` instead of `env`), requiring a format transformer in the runtime code for that platform.
...
- **Env var overrides**: environment variables that override default binary or config paths (e.g., `OPENCODE_INSTALL_DIR`, `XDG_CONFIG_HOME`)
...
**Adding a new platform**: Adding support for platform 8+ requires adding a JSON entry to `platforms.config.json` for standard-format platforms. No code changes, no new adapter modules, no new TypeScript files. The entry specifies binary name, config paths per OS, MCP config location, root key, env var overrides, and content directories. Platforms with non-standard MCP formats (e.g., OpenCode uses command arrays instead of command+args and `environment` instead of `env`) require a format transformer in the runtime code in addition to the JSON entry.
...
**Env var overrides**: 3 of 7 platforms use environment variables to override default binary or config paths. The `platforms.config.json` schema includes an `envOverrides` field per platform that lists environment variables to check for non-standard paths. Detection expands these variables before checking paths.

- OpenCode: `OPENCODE_INSTALL_DIR`, `XDG_BIN_DIR`, `OPENCODE_BIN_PATH`
- Amp: `XDG_CONFIG_HOME` (affects config path on all OSes including Windows)
- Multiple platforms: `XDG_CONFIG_HOME`, `XDG_DATA_HOME` (for XDG-compliant platforms)

**Schema example** (`platforms.config.json`, showing 2 of 7 platforms):
  ...
  "platforms": {
    ...
    "opencode": {
      ...
      "mcpConfig": {
        ...
        "rootKey": "mcp",
        "formatTransformer": "opencode"
      },
      "envOverrides": {
        "binaryPath": ["OPENCODE_BIN_PATH"],
        "installDir": ["OPENCODE_INSTALL_DIR"],
        "binDir": ["XDG_BIN_DIR"]
      },
      "contentDirs": {
...
Platforms requiring non-standard MCP object format include a `formatTransformer` field in `mcpConfig`. The runtime dispatches to a named transformer function that converts the standard `{ command, args, env }` shape to the platform's expected format.
...
- **POS-001**: Dual detection achieves high reliability across all 7 platforms. GUI editors without PATH binaries are caught by directory check. Uninstalled tools with leftover directories are confirmed by binary check. ANALYSIS-026 rates overall detection reliability as "High" for all 7 platforms.
- **POS-002**: Parallel detection via `Promise.allSettled` with 3-second timeouts bounds total detection time. One slow check does not block the other 6.
...
- **POS-005**: `platforms.config.json` reduces the cost of adding a standard-format platform from "write a TypeScript module with tests" to "add a JSON entry." Platforms with non-standard MCP formats (like OpenCode) additionally require a format transformer in the runtime code. Community contributors can add standard-format platform support without understanding runtime internals.
...
- Neutral, because spawns 7 subprocesses for binary checks (negligible overhead)
...
- Bad, because misses GUI editors (Cursor, Windsurf) where user has not installed shell command
- Bad, because ANALYSIS-026 rates Cursor and Windsurf binary-in-PATH likelihood as "Medium"
...
- Bad, because forces plugin authors to track 7 platforms' path conventions
...
- **IMP-005**: The MCP key `plugin-name:server-name` is written into the platform's root key object. For Amp, this means `amp.mcpServers["plugin-name:server-name"]`. For Claude Code, this means `mcpServers["plugin-name:server-name"]`. Three root key variants exist across 7 platforms: `mcpServers` (5 platforms), `mcp` (1 platform: OpenCode), `amp.mcpServers` (1 platform: Amp).
- **IMP-006**: MCP server objects contain `command`, `args`, and `env` fields. These are identical across 6 of 7 platforms. OpenCode uses a non-standard format: command as an array (instead of separate command+args) and `environment` instead of `env`. Platforms with non-standard formats require a `formatTransformer` entry in `platforms.config.json` that dispatches to a named transformer function in the runtime code.
...
1. Integration tests running dual detection on all 7 platforms across macOS, Linux, and Windows CI runners
...
- [decision] Dual platform detection adopted: binary check via which/where AND config directory existence check, run in parallel for all 7 platforms #platform-detection #reliability
...
- [decision] Adding a standard-format platform requires only a JSON entry in platforms.config.json. Non-standard-format platforms (e.g., OpenCode) additionally require a format transformer in runtime code #extensibility #data-driven
- [fact] 7 target platforms use 3 different MCP root keys: mcpServers (5 platforms), mcp (1: OpenCode), amp.mcpServers (1: Amp) #mcp #platform-diversity
- [fact] OpenCode uses non-standard MCP format: command array instead of command+args, environment instead of env. Requires format transformer. #platform-divergence #opencode
- [fact] Claude Code supports both project-level (.mcp.json) and user-level (~/.claude.json) MCP configuration #claude-code #mcp-scope
- [fact] 3 of 7 platforms use env var overrides for binary/config paths: OpenCode (OPENCODE_INSTALL_DIR, XDG_BIN_DIR, OPENCODE_BIN_PATH), Amp (XDG_CONFIG_HOME), and XDG-compliant platforms #env-overrides #detection
- [fact] GUI editors Cursor and Windsurf have Medium likelihood of binary in PATH, requiring directory fallback for reliable detection #detection-reliability
